...
import { Dimensions, ScrollView, TextInput, View, useColorScheme } from 'react-native'
...
import ChevronRightIcon from '@hugeicons/TaskDone01Icon'
...
export default function BookingOTPVerification() {
  ...
  const { status = 'confirmed' } = (route.params || {}) as {
  ...
  return (
    <View className='flex-1 bg-white'>
      <HybridHead title='Booking Verification' showBackButton={true} onBackPress={() => navigation.goBack()} />
      ...
      {/* Payment Footer */}
      <View className='border-t border-neutral-200 bg-white px-5 py-3 dark:border-neutral-700 dark:bg-zinc-900'>
        <View className='flex-row items-center justify-between'>
          <View className='gap-1'>
            <SemiBold className='text-lg text-neutral-900 dark:text-white'>₹{appointmentData.payment.total}</SemiBold>
            <Press onPress={() => {}}>
              <Medium className='text-sm text-blue-600 dark:text-blue-400'>View Detailed Bill</Medium>
            </Press>
          </View>
          <Press
            onPress={() => setShowOtpSheet(true)}
            className='flex-row items-center gap-3 rounded-2xl bg-blue-600 px-6 py-3 dark:bg-green-700'
          >
            <SemiBold className='text-white'>Collect & Verify</SemiBold>
            <ChevronRightIcon size={18} color='#fff' strokeWidth={2} />
          </Press>
        </View>
      </View>
      ...
      <BottomSheetCustom visible={showOtpSheet} onClose={() => setShowOtpSheet(false)} height={SCREEN_HEIGHT * 0.35}>
        <View className='gap-4 px-5 py-6'>
          <SemiBold className='text-lg text-neutral-900 dark:text-white'>Enter Verification Code</SemiBold>
          <Medium className='text-sm text-neutral-600 dark:text-neutral-400'>
            Patient needs to provide the 4-digit verification code
          </Medium>
          ...
          <View className='mt-2 gap-3'>
            <Button title='Verify & Collect' onPress={() => setShowOtpSheet(false)} disabled={otp.length !== 4} />
